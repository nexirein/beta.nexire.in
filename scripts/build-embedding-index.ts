 ...
 * Phase 4 — One-time script to populate Supabase pgvector with Prospeo enum embeddings.
 *
 * HOW TO USE:
 * 1. Paste the TECHNOLOGIES and INDUSTRIES arrays below (from Prospeo docs)
 * 2. Make sure SUPABASE_SERVICE_ROLE_KEY is in .env.local
 * 3. Run: npx tsx scripts/build-embedding-index.ts
 * 3. Run: npx tsx scripts/build-embedding-index.ts
 *
 * ─── HOW TO GET THE LISTS ───────────────────────────────────────────────────
 * Technologies: https://prospeo.io/api-docs/enum/technologies → copy the array
 * Industries:   https://prospeo.io/api-docs/enum/industries → copy the array
 * Paste each into the arrays below, then run this script.
 * ────────────────────────────────────────────────────────────────────────────
 */
...
// ════════════════════════════════════════════════════════════════════════════
// 1. PASTE YOUR TECHNOLOGY STRINGS HERE (from Prospeo API docs)
//    Example: ["React", "Python", "AWS Lambda", ...]
// ════════════════════════════════════════════════════════════════════════════
...
// ════════════════════════════════════════════════════════════════════════════
// 2. PASTE YOUR INDUSTRY STRINGS HERE (from Prospeo API docs)
//    Example: ["Software Development", "Financial Services", ...]
// ════════════════════════════════════════════════════════════════════════════
...
// ════════════════════════════════════════════════════════════════════════════
// 3. PASTE YOUR DEPARTMENT STRINGS HERE (from Prospeo API docs)
//    Example: ["Engineering & Technical", "Artificial Intelligence / Machine Learning", ...]
// ════════════════════════════════════════════════════════════════════════════
...
async function embedBatch(texts: string[]): Promise<{ text: string; embedding: number[] }[]> {
  ...
  for (let i = 0; i < texts.length; i += CHUNK) {
    ...
    const payload = {
      requests: chunk.map(c => ({
        ...
        content: { parts: [{ text: c.trim().toLowerCase() }] },
    ...
    if (!data.embeddings) {
      console.error("Batch embedding failed:", data.error?.message || "Unknown error");
      throw new Error("Batch embedding failed");
    }

    data.embeddings.forEach((emb: any, idx: number) => {
      results.push({ text: chunk[idx], embedding: emb.values });
    });

    process.stdout.write(`\r  Embedding: ${Math.min(i + CHUNK, texts.length)}/${texts.length}`);
...
async function buildIndex(
  ...
  if (labels.length === 0) {
    console.log(`⚠️  ${category}: Array is empty — skipping.`);
    return;
  }
  ...
  console.log(`💾 Upserting into Supabase pgvector...`);
  const BATCH = 100;
  let inserted = 0;
  ...
  for (let i = 0; i < embedded.length; i += BATCH) {
    ...
    process.stdout.write(`\r  Upserted: ${inserted}/${embedded.length}`);
  }
...
async function main() {
  console.log("🚀 Nexire Phase 4 — Embedding Index Build");
  ...
  // Verify connection
  const { error: connErr } = await supabase.from("prospeo_embeddings").select("id").limit(1);
  if (connErr) {
  ...
  await buildIndex(supabase, "technology", TECHNOLOGIES);
  await buildIndex(supabase, "industry", INDUSTRIES);
  await buildIndex(supabase, "department", DEPARTMENTS);
  ...
  console.log(`\n🎉 Index build complete! Total rows in prospeo_embeddings: ${count}`);
  console.log("   AI tech/industry resolution is now active in /api/ai/extract-and-resolve");
}
