...
export function ShortlistClient() {
  ...
  return (
    <div className="flex h-full w-full relative overflow-hidden bg-gray-50/50">
      <div 
        className={cn(
          "flex flex-col h-full bg-[#FAFAFA] text-gray-900 border-l border-gray-100/60 overflow-y-auto transition-all duration-300 ease-in-out",
          isDrawerOpen ? "w-[45%] xl:w-[45%] border-r border-gray-200" : "w-full"
        )}
      >
        {/* HEADER */}
              ...
              {entries.map((entry) => {
                ...
                return (
                  <div 
                    ...
                    className={cn(
                      "group bg-white border border-gray-200 hover:border-indigo-300 rounded-xl p-5 flex items-start gap-5 cursor-pointer transition-all hover:shadow-md hover:shadow-indigo-500/5",
                      selectedCandidate?.person_id === c.person_id && isDrawerOpen ? "border-indigo-400 bg-indigo-50/30" : ""
                    )}
                  >
